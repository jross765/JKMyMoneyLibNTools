...
public class FilePriceManager {
	...
	LOGGER.debug("getLatestPrice: pSecCurrId='"
     ...
     * @see #getDefaultCurrencyID()
     */
    private FixedPointNumber getLatestPrice_readAfresh(final KMMQualifSecCurrID secCurrID, final int depth) throws InvalidQualifSecCurrIDException, InvalidQualifSecCurrTypeException {
	if (secCurrID == null) {
	    throw new IllegalArgumentException("null parameter 'secCurrID' given");
	}
	// System.err.println("depth: " + depth);
	...
	for ( PRICEPAIR pricePair : priceDB.getPRICEPAIR() ) {
	    ...
	    for ( PRICE prc : pricePair.getPRICE() ) {
		    if (prc == null) {
			LOGGER.warn("getLatestPrice_readAfresh: KMyMoney file contains null price-quotes - there may be a problem with JWSDP");
			continue;
		    }
		...
		try {
			...
			LOGGER.warn("getLatestPrice_readAfresh: KMyMoney file contains price-quotes without price value: "
				+ "(" + pricePair.getFrom() + "/" + pricePair.getTo() + ")");
			continue;
		    }
		    ...
		    if ( toCurr.startsWith("E0") ) {
		      // is security
			if ( depth > maxRecursionDepth ) {
			    LOGGER.warn("getLatestPrice_readAfresh: Ignoring price-quote that is not in an ISO4217-currency"
				    + " but in '" + toCurr + "'");
			    continue;
			}
			...
			if ( ! toCurr.equals(kmmFile.getDefaultCurrencyID()) ) {
			    if ( depth > maxRecursionDepth ) {
				LOGGER.warn("getLatestPrice_readAfresh: Ignoring price-quote that is not in " + kmmFile.getDefaultCurrencyID()
					+ " but in '" + toCurr + "'");
				continue;
			    }
			    factor = getLatestPrice(new KMMQualifCurrID(toCurr), depth + 1);
			...
			LOGGER.debug("getLatestPrice_readAfresh: pSecCurrId='" + secCurrID.toString()
	...
	LOGGER.debug("getLatestPrice_readAfresh: pSecCurrId='"
		+ secCurrID.toString() + "'= " + latestQuote + " from " + latestDate);
