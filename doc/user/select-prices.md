...
* *Technical level*: Prices have no technical IDs in KMyMoney. Instead, 
  they are technically selected with a (pseudo-)technical ID, constisting of a 
...
in KMyMoney before reading the next section; otherwise it will probably confuse you.
...
* Specifying the security by its (pseudo-)technical ID (directly), using the syntax 
...
* Specifying the security by its business logic ID (the field "Code" in KMyMoney lingo), 
  i.e. its ISIN, CUSIP, SEDOL or similar official security identifiers (without name-space prefix).
  
  This approach obviously only works when you actually have filled the field "Code".
